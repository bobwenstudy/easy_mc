...
#define TIM_ENCODER_COUNT htim4
#define UART_VOFA_DEBUG   huart2
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    {
        // EASY_MC_LOG_DBG("PA0: %d, PA1: %d, PA6: %d, PA5: %d\n", data[0], data[1], data[2], data[3]);
        float vbus = (float)adc2_data[0] / 4096 * 3.3f * (3 + 75) / 3;

        // EASY_MC_LOG_DBG("vbus: %.3f(%d), temp: %.3f\n", vbus, adc2_data[0], temp);
...
void HAL_ADCEx_InjectedConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    if (hadc == &hadc1)
    {
        ...
        motor_control.ib_adc = HAL_ADCEx_InjectedGetValue(&hadc2, ADC_INJECTED_RANK_1);
        motor_control.ic_adc = HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_2);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    {
        // Uq Open Loop
        // easy_mc_user_set_u_ref(-24, 0);
        // easy_mc_user_set_u_ref(0, 10);
        // Current Loop
        easy_mc_user_set_i_ref(0.3, 0);
        // easy_mc_user_set_i_ref(0, 1);
        // Speed Loop
        // easy_mc_set_speed_ref(-200);
        easy_mc_user_motor_control(true);
    }
    if (Button2_Pin == GPIO_Pin)
    {
        // Stop motor
        easy_mc_user_motor_control(false);
    }
...
void easy_mc_hw_init(void)
{
    ...
    // Enable OPAMP, use get three phase current.
    HAL_OPAMP_Start(&hopamp1);
    HAL_OPAMP_Start(&hopamp2);
    HAL_OPAMP_Start(&hopamp3);

    // Enable calibration ADC
